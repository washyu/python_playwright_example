...
import pytest
from models import LoginPage, InventoryPage, CartPage
...
def test_happy_path_complete_purchase(page: Page, test_data):
    ...
    assert "Sauce Labs Backpack" in item_names, "Cart should contain Sauce Labs Backpack"

    # Verify item details
    item_details = cart_page.get_item_details(0)
    assert item_details['name'] == test_data['products']['sauce_labs_backpack']['name'], "Item name should be Sauce Labs Backpack"
    assert str(test_data['products']['sauce_labs_backpack']['price']) in item_details['price'], "Item price should be $29.99"
    assert item_details['description'] ==test_data['products']['sauce_labs_backpack']['description'],  "Item should have a description"
    ...
    assert test_data['products']['sauce_labs_backpack']['name'] in order_items, "Order should contain Sauce Labs Backpack"
    ...
    assert subtotal == expected_subtotal, f"Item total should be ${expected_subtotal}, got ${subtotal}"

    # Calculate expected tax using the tax rate
    expected_tax = round(expected_subtotal * tax_rate, 2)
    assert tax == expected_tax, f"Tax should be ${expected_tax} ({tax_rate * 100}% of ${expected_subtotal}), got ${tax}"
    ...
    assert payment_info == expected_payment, f"Payment info should be '{expected_payment}', got '{payment_info}'"
    assert shipping_info == expected_shipping, f"Shipping info should be '{expected_shipping}', got '{shipping_info}'"
    ...
    assert header_text == expected_success_header, f"Header should be '{expected_success_header}', got: {header_text}"
    ...
    assert expected_success_message in confirmation_text, f"Confirmation should contain '{expected_success_message}', got: {confirmation_text}"

    # Verify pony express image is visible (order dispatched indicator)
    assert checkout_complete.is_pony_express_visible(), "Order dispatch image should be visible"
    ...
    assert not inventory_page.is_cart_badge_visible(), "Cart should be empty after purchase"
