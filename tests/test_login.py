...
def test_login(page: Page, test_users):
    ...
    assert (
        product_count > 0
    ), f"Expected products to be displayed, found {product_count}"
...
def test_blocked_login(page: Page, test_users):
    ...
    assert (
        "locked out" in error_text.lower()
...
def test_no_username(page: Page):
    ...
    error_text = login_page.get_error_text()
    assert (
        "username" in error_text.lower()
    ), f"Expected 'Username' in the error message, got: {error_text}"
...
def test_no_password(page: Page):
    ...
    error_text = login_page.get_error_text()
    assert (
        "password" in error_text.lower()
    ), f"Expected 'Password' in the error message, got: {error_text}"
...
def test_invalid_username(page: Page):
    ...
    error_text = login_page.get_error_text()
    assert (
        "do not match" in error_text.lower() or "not match" in error_text.lower()
    ), f"Expected 'not match' in error message, got: {error_text}"
