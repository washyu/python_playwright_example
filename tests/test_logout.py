...
def test_cannot_access_inventory_after_logout(page: Page, test_users):
    ...
    expect(page).to_have_url(
        re.compile(".*saucedemo.com/?$|.*saucedemo.com/inventory.html")
    )
    ...
    if "inventory.html" in page.url:
        ...
        ), "Error message should be shown when accessing inventory without login"
    else:
...
def test_menu_links_accessible_when_open(page: Page, test_users):
    ...
    assert (
        inventory_page.menu_close_button.is_visible()
    ), "Menu close button should be visible when menu is open"
    ...
    assert (
        inventory_page.all_items_link.count() > 0
    ), "All Items link should exist in menu"
